function DiffWrapper() {
    ...
    diff.prototype.cleanupSemanticLossless = function (diffs) {
        ...
        function diff_cleanupSemanticScore_(one, two) {
            ...
            var whitespace1 =
                nonAlphaNumeric1 && char1.match(diff.whitespaceRegex_);
            var whitespace2 =
                nonAlphaNumeric2 && char2.match(diff.whitespaceRegex_);
...
class TextCompareService {
    ...
    compare(text1, text2) {
        ...
        const Diff = DiffWrapper();
        const diff = new Diff();

        textDiff = diff.main(text1, text2);
        diff.cleanupSemantic(textDiff);

        textDiff = this.splitSubarray(textDiff);
